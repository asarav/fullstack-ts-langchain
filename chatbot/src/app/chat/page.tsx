...
export default function Home() {
      ...
      for (let i = 0; i < chatHistory.length; i++) {
        ...
        if (entry.role === "user") {
          ...
          if (
            ...
            chatLogEntries[chatLogEntries.length - 1].response === ""
          ) {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      console.log(response);
      const responseText =
        response?.data?.messages[response.data?.messages.length - 1]?.kwargs
          ?.content;
  ...
  return (
    <div className="items-center justify-items-center min-h-screen p-8 pb-20 gap-16 sm:p-20 font-[family-name:var(--font-geist-sans)] pl-4 pr-4">
      <main className="flex flex-col row-start-1 sm:items-start w-full">
        <div className="chat-log w-full bg-gray-950 p-4 rounded-lg h-140 overflow-y-auto border border-gray-700">
